...
from .backbone_2048 import test_1_pre,test_2_end,test_3_pre,test_4_pre,CNN4_b,CNN1_a,CNN3_c,CNN2_d
# from random_erasing import RandomErasing_vertical, RandomErasing_2x2
import math
import copy

def make_model(args):
    return M123_2048(args)
######################################################################
...
class M123_2048(nn.Module):
    def __init__(self, num_classes, num_stripes=4, local_conv_out_channels=32, feats=256 ,erase=0, loss={'softmax'}, avg=False, **kwargs):
        ...
        # self.maxpool_zg_p2 = nn.MaxPool2d(kernel_size=(12, 8))
        # self.maxpool_zg_p3 = nn.MaxPool2d(kernel_size=(8, 8))
    ...
    def forward(self, x):
        ...
        # zg_p1 = self.maxpool_zg_p1(feats_a)
        # zg_p2 = self.maxpool_zg_p2(feats_a)
        # zg_p3 = self.maxpool_zg_p3(feats_d)      

        p1 = self.avgpool_p1(feats_a)


        p2 = self.avgpool_p2(feats_a)
        z0_p2 = p2[:, :, 0:1, :]
        z1_p2 = p2[:, :, 1:2, :]
